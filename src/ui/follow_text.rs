...
pub struct SpawnFollowText<T: Component> {
        ...
        if let Ok(mesh_position) = mesh_query.get(follow_text.following) {
            ...
            for (camera, camera_transform) in camera_query.iter() {
                ...
                match camera.world_to_viewport(camera_transform, translation) {
                    Some(coords) => {
                        style.left = Val::Px(coords.x - text_layout_info.logical_size.x / 2.0);
                        style.top = Val::Px(coords.y - text_layout_info.logical_size.y / 2.0);
                    }
                    None => {
